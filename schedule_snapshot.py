...
CATALOG = {
    "com.jalal.aoife-library-reminder": {
        ...
        "what": "Daily 07:05 Telegram nudge to the 📡 alerts chat reminding Jalal the concierge can place/swap Warwick library holds for Aoife (project memory project-aoife-library-holds). Runs 2026-09-02 → 2026-09-29 then unloads itself",
        "logs": "~/Library/Logs/aoife-library-reminder.log",
        "notes": "TEMPORARY by design — a habit-forming nudge, not a monitor. Deliberately daytime (07:05, next to the 7:00 budget brief) because a human has to read it. Self-terminates after Sep 29 2026 via launchctl bootout inside the script; expect Frequency=Removed after that. Do not add a fleet-health probe"},
    "com.jalal.mac-audit": {
