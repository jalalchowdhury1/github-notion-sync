...
CATALOG = {
    ...
    "com.jalal.overnight-blackout-check": {
        "title": "Overnight blackout check — logged-out-Mac alarm",
        "what": "Fires on login (RunAtLoad, no calendar interval) and, once between 06:00-22:00, checks whether every overnight job (Dhaka flights, hotel rates, CarMax, catalyst radar, fleet health, daily trackers, mac-audit, Aoife calendar sync) has a run dated today. For any missing API-only job (catalyst radar, Aoife calendar sync, mac-audit, fleet health) it self-heals by running that job's own script straight away, into its normal log, so fleet-health still grades it. Chrome-driving jobs (Dhaka flights, hotel rates, CarMax) are never auto-run. Whatever is still missing after self-heal gets ONE loud Telegram message to the alerts chat naming it plus the replay command",
        "logs": "~/Library/Logs/overnight-blackout-check.log",
        "notes": "Added 2026-09-11 after macOS logged the user out at 23:57 to install 15.7.9, the install failed with a boot-policy error, the Mac restarted, and FileVault held it at the login screen until 09:01 — every overnight job is a *user* LaunchAgent and needs a logged-in session, so none of them ran. The non-obvious trap: launchd only replays a missed StartCalendarInterval when the agent was already loaded and the machine merely slept; with no session at midnight the agents did not exist at all, so when they finally loaded at 09:01 they just scheduled their next run for tonight — the whole night vanished with no error anywhere, and fleet-health, the thing that would normally have caught it, was itself a user agent and died with everything else. EVENT-DRIVEN, not scheduled — it has no calendar slot to grade, and it is SILENT on every normal day (session loads, all jobs already ran, nothing sent), so like the temporary reminder jobs above, do NOT add a fleet-health probe: a probe here would false-alarm on every quiet day. The self-heal split is deliberate: light jobs are plain API calls that finish in a couple of minutes and disturb nothing, so there is no reason to make Jalal ask for them; the heavy Chrome-driving scrapers take 10-35 min and would seize the machine mid-morning — standing rule is no daytime scheduling, so those always wait for Jalal. Companion manual command: `catchup-overnight` (~/.local/bin/catchup-overnight.sh, aliased in ~/.zshrc), which Jalal runs himself to replay whatever is still outstanding"},
    "com.jalal.toolcheck": {
    ...
    "com.jalal.job-reaper": {
        "title": "Job reaper — stops hung scheduled jobs (every 5 min)",
        "what": "Runs ~/.local/bin/job-reaper.py at :02, :07 … :57. Any scheduled job in its CAPS_MIN table that launchd still reports as running past its cap gets its process group TERMed, KILLed 30 s later if needed, and one silent note goes to the alerts thread.",
        "logs": "~/Library/Logs/job-reaper.log (stderr: job-reaper.err)",
        "notes": "Added 2026-09-15 after mental-models hung inside an OpenRouter call. launchd never starts a second copy of a job that is still running, so one hung run silently blocks every later slot. It is a central reaper rather than a wrapper in each plist because TCC grants follow the spawned binary: t7sync is a signed Mach-O and would lose Full Disk Access behind a bash wrapper. KeepAlive daemons and tranche-deepredteam are deliberately uncapped. When adding a scheduled job, add its cap to CAPS_MIN. `--dry-run` reports without killing. Silent on normal passes; fleet-health wants a clean pass within the last hour.",
    },
}
